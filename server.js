...
app.get('/api/activities', (req, res) => {
  const indexPath = path.join(ACTIVITIES_DIR, 'index.json');
  if (!fs.existsSync(indexPath)) return res.json([]);
  try { res.json(JSON.parse(fs.readFileSync(indexPath, 'utf8'))); }
  catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/activities/:id', (req, res) => {
  const id = req.params.id.replace(/[^a-zA-Z0-9T:\-]/g, '');
  const p = path.join(ACTIVITIES_DIR, `activity_${id}.json`);
